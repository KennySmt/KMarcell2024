...
public class PathWalker : MonoBehaviour
{
    ...
    Vector3 GetPoint(int index) => useGlobalSpace ? transform.TransformPoint(points[index]) : points[index];
    ...
    void Update()
    {
        ...
        if (movingObject.position == target)
        {
            ...
            {
                if ((moveForward && index == points.Length-1) || (!moveForward) && index == 0) moveForward = !moveForward;
    ...
    void OnDrawGizmosSelected()
    {
        ...
        if(isLooping && points.Length >=3) Gizmos.DrawLine(GetPoint(0), GetPoint(points.Length-1)); //^1 = points.length -1
    }
}
